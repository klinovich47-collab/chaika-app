...
function concertGroupKey(c){return `${String(c.artist||'').toLowerCase().replace(/[^a-zа-я0-9]+/giu,' ').trim()}|${String(c.venue||'').toLowerCase().replace(/[^a-zа-я0-9]+/giu,' ').trim()}|${String(c.url||'').toLowerCase().replace(/[?#].*$/,'')}`}
function groupedConcerts(list){const groups=new Map();list.forEach(c=>{const key=concertGroupKey(c),group=groups.get(key);if(group){if(!group.dates.some(d=>d.getTime()===c.date.getTime()))group.dates.push(c.date);if(!group.image&&c.image)group.image=c.image}else groups.set(key,{...c,dates:[c.date]})});return [...groups.values()].map(c=>({...c,dates:c.dates.sort((a,b)=>a-b),date:c.dates.sort((a,b)=>a-b)[0]})).sort((a,b)=>a.date-b.date)}
function concertGenreLabel(c){const text=String(c.artist||'').toLowerCase();if(/стендап|stand[ -]?up/.test(text))return'СТЕНДАП';return({electronic:'ЭЛЕКТРОНИКА',rock:'РОК',pop:'ПОП',other:'СОБЫТИЕ'})[c.genre]||'СОБЫТИЕ'}
function concertDatesLabel(c){const first=c.dates[0],date=first.toLocaleDateString('ru-RU',{day:'numeric',month:'long',year:'numeric',timeZone:'Europe/Moscow'}),time=first.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit',timeZone:'Europe/Moscow'});const rest=c.dates.length-1;if(!rest)return`${date} · ${time}`;const word=rest%10===1&&rest%100!==11?'дата':rest%10>=2&&rest%10<=4&&(rest%100<12||rest%100>14)?'даты':'дат';return`${date} · ${time}<br>Ещё ${rest} ${word}`}
function renderConcerts(){const list=groupedConcerts(concerts.filter(c=>state.concertGenre==='all'||c.genre===state.concertGenre));$('concertList').innerHTML=list.map(c=>{const photo=c.image?`<img class="concert-photo" data-concert-image src="${escapeHtml(c.image)}" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">`:'';return `<article class="concert-card"><div class="concert-visual ${photo?'has-photo':''}">${photo}<span class="badge">${concertGenreLabel(c)}</span><strong>${escapeHtml(c.artist)}</strong></div><div class="concert-body"><div class="concert-meta">${concertDatesLabel(c)}<br>${escapeHtml(c.venue)}</div><div class="concert-bottom"><b>${escapeHtml(c.price)}</b><button data-buy="${c.id}">Купить билет ↗</button></div></div></article>`}).join('');$('concertList').querySelectorAll('[data-concert-image]').forEach(img=>img.addEventListener('error',()=>{img.closest('.concert-visual')?.classList.remove('has-photo');img.remove()},{once:true}));$('concertList').querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>{const c=concerts.find(x=>String(x.id)===String(b.dataset.buy));if(!c)return;if(tg?.openLink)tg.openLink(c.url);else window.open(c.url,'_blank','noopener,noreferrer')})}
...
function chaikaManagedEventCard(row,admin=false){
  ...
  const isOwn=Number(row.telegram_owner_id)===Number(chaikaAuth.user?.id),selfBlocked=admin&&isOwn&&chaikaManagement.isModerator&&!chaikaManagement.isAdmin;
  const adminButtons=admin?`${status!=='published'&&!selfBlocked?`<button class="approve" data-admin-decision="published" data-event-id="${row.id}">Одобрить</button>`:''}${status!=='rejected'?`<button data-admin-decision="rejected" data-event-id="${row.id}">Отклонить</button>`:''}`:'';
  const creator=admin&&row.telegram_owner_id?`<div class="chaika-manage-creator">Автор: ${escapeHtml(row.creator_username?'@'+row.creator_username:(row.creator_name||'Пользователь'))}</div>`:'';
  const description=admin?`<p class="chaika-manage-description"><b>Описание:</b> ${escapeHtml(row.description||'Не заполнено')}</p>`:'';
  const selfNote=selfBlocked?'<div class="chaika-self-review-note">Своё событие должен проверить другой модератор или администратор.</div>':'';
  return `<article class="chaika-manage-item"><div class="chaika-manage-top"><div><div class="chaika-manage-title">${escapeHtml(row.title)}</div><div class="chaika-manage-meta">${eventListMeta(e)}</div>${creator}</div><span class="chaika-status ${status}">${chaikaStatusLabel(status)}</span></div>${description}${selfNote}<div class="chaika-manage-actions"><button data-managed-open="${row.id}" data-managed-source="${admin?'admin':'mine'}">Открыть</button>${adminButtons}<button class="danger" data-managed-delete="${row.id}">Удалить</button></div></article>`;
}
...
async function chaikaModerateManagedEvent(eventId,decision){
  const row=chaikaFindManagedRow(eventId,'admin');
  if(decision==='published'&&!window.confirm(`Одобрить событие после проверки обоих полей?\n\nНазвание: ${row?.title||''}\nОписание: ${row?.description||'Не заполнено'}`))return;
  try{
    ...
    await Promise.all([chaikaLoadEvents(false),chaikaLoadManagement(false)]);
  }catch(error){console.error('CHAIKA moderate event',error);toast(error.message==='self_moderation_forbidden'?'Своё событие должен проверить другой модератор':error.message==='forbidden'?'Нужны права модератора':'Не удалось изменить статус')}
}
...
const chaikaDangerBlockPatterns=[/наркот|закладк|героин|кокаин|амфетамин|мефедрон|метамфетамин/i,/оружи|боеприпас|взрывчат|бомб[ау]|террор|экстрем/i,/массов\S*\s+(убий|расстрел|резн)|массовое\s+убийство/i,/убийств|убить\s+(люд|человек|кого|всех)|расстрел|резн[яи]|пытк|казн[ьи]|линч/i,/жертвопринош|человеческ\S*\s+жертв|ритуальн\S*\s+убий|принос\S*\s+жертв/i,/изнасил|сексуальн\S*\s+насили|\bцп\b/i,/самоубий|суицид|прыгн\S*\s+с\s+(крыши|моста)|вскрыть\s+вен/i,/убить\s+(кот|кош|собак|живот)|мучить\s+(кот|кош|собак|живот)|издев\S*\s+над\s+(кот|кош|собак|живот)|живодер/i,/(ку\s*клукс?|куклус)\s*клан|майн\s*кампф|австрийск\S*\s+художник|россия\s+для\s+русских|только\s+для\s+русских/i,/купить\s+паспорт|продам\s+паспорт/i];
const chaikaReviewPatterns=[/\b(хуй|хуя|хуе|пизд|ебан|ебат|бляд)\S*/i,/эротич|маньяк|какаш|фекал|дерьм|говн|нацист|свастик|гитлер/i,/без\s+правил|секретн\S*\s+адрес|только\s+налич|100%\s+заработ|легк\S*\s+деньг/i,/по\s+приколу|рофл|прикол\S*\s+событ|поюзат\S*\s+(кот|кош|живот)/i,/драка|подраться|мордобой|охот\S*\s+на\s+люд/i,/кровав\S*\s+(вечерин|ритуал)|сатанин\S*\s+ритуал/i];
