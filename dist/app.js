...
function chaikaPlainText(value){const raw=String(value||'').trim();if(!raw)return '';const decoder=document.createElement('textarea');decoder.innerHTML=raw;const decoded=decoder.value;const parsed=new DOMParser().parseFromString(decoded,'text/html');return String(parsed.body?.textContent||decoded).replace(/\s+/g,' ').trim()}
function chaikaEvent(row){const start=chaikaDateParts(row.starts_at),mine=state.going.has(row.id);return {id:row.id,title:chaikaPlainText(row.title),category:row.category,date:start.date,time:start.time,price:Number(row.price_rub||0),venue:chaikaPlainText(row.venue),lat:Number(row.lat),lng:Number(row.lng),ageLimit:Number(row.age_limit||0),promoted:Boolean(row.promoted),description:chaikaPlainText(row.description),ticketUrl:row.ticket_url||'',imageData:row.image_url||'',going:Math.max(0,Number(row.going_count||0)-(mine?1:0)),owner:state.createdIds.has(row.id),source:row.source||null,type:row.event_type||'planned',startsAt:new Date(row.starts_at).getTime(),expiresAt:row.expires_at?new Date(row.expires_at).getTime():null,moderationStatus:row.moderation_status||'published'}}
