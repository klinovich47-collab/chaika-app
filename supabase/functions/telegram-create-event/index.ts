...
const homoglyph:Record<string,string>={a:'а',c:'с',e:'е',o:'о',p:'р',x:'х',y:'у',k:'к',m:'м',t:'т',h:'н',b:'в'};
function norm(raw:string){let s=String(raw||'').normalize('NFKC').toLowerCase().replace(/ё/g,'е').replace(/[\u200B-\u200D\uFEFF]/g,'');s=s.replace(/[0346@$]/g,ch=>({'0':'о','3':'з','4':'ч','6':'б','@':'а','$':'с'}[ch]||ch));const mixed=s.replace(/[aceopxykmthb]/g,ch=>homoglyph[ch]||ch);const spaced=mixed.replace(/[^a-zа-я0-9]+/giu,' ').replace(/\s+/g,' ').trim();return {spaced,compact:spaced.replace(/\s+/g,'')};}
function hasAny(v:string,terms:string[]){return terms.some(t=>v.includes(t));}
function localModeration(raw:string){
  const {spaced,compact}=norm(raw);
  const hardTerms=['наркот','закладк','кладмен','героин','кокаин','амфетамин','метамфетамин','мефедрон','марихуан','каннабис','экстази','псилоциб','оружи','боеприпас','взрывчат','террор','экстрем','проституц','интимуслуг','сексзаденьги','массовоеубий','массовыйрасстрел','жертвопринош','человеческаяжертв','ритуальноеубий','изнасил','сексуальноенасили','самоубий','суицид','живодер'];
  const hardPatterns=[/массов\S*\s+(убий|расстрел|резн)/u,/убить\s+(люд|человек|кого|всех)/u,/(расстрел|резн[яи]|пытк|казн[ьи]|линч)/u,/(убить|мучить|издев\S*)\s+(кот|кош|собак|живот)/u,/(прыгн\S*\s+с\s+(крыши|моста)|вскрыть\s+вен)/u,/(юз\S*|поюз\S*)\s+(кот|кош|котик)/u,/(упорот|вмаз|ширнут|снюх|под\s+веществ)/u,/(дорожк\S*\s+(кокс|кокаин)|колоть\s+(героин|наркот))/u,/(сексуал\S*|секс)\s+.*(дет|ребен|подрост|несовершеннолет)/u];
  if(hasAny(compact,hardTerms)||hardPatterns.some(r=>r.test(spaced)))return {status:'block',reason:'dangerous_or_illegal'} as const;
  const reviewTerms=['хуй','хуя','хуе','хер','пизд','пиздец','ебан','ебат','ебля','ебуч','ебнут','уеб','заеб','наеб','поеб','выеб','бляд','шлюх','манда','елда','залуп','фаллос','фалос','пенис','писюн','письк','вагин','вареник','сперм','конч','дроч','отсос','минет','куни','порно','оргия','сиськ','титьк','жоп','анус','секс','анал','член','hui','huy','pizda','pizdec','ebat','eblya','blyad','blyat','chlen','zalupa','dick','cock','pussy','blowjob','porn','fuck','sex','шмаль','гашиш','травка','косяк','спиды','скорость','кислота','таблы','колеса','кокс','эскорт','драка','подраться','мордобой','охотаналюд','кровавыйритуал','сатанинскийритуал','безправил','секретныйадрес','тольконалич','легкиеденьги','100заработ'];
  const reviewPatterns=[/(по\s+приколу|рофл|прикол\S*\s+событ)/u,/(кур\S*|забить|пыхн\S*)\s+.*(шмаль|трав|косяк|гаш)/u,/(поюз\S*|юз\S*)\s+.*(веществ|табл|колес|скорост)/u];
  if(hasAny(compact,reviewTerms)||reviewPatterns.some(r=>r.test(spaced)))return {status:'review',reason:'slang_or_ambiguous'} as const;
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const allText=`Название: ${title}\nОписание: ${description}\nМесто: ${venue}`;const local=localModeration(allText);if(local.status==='block')throw new Error('event_blocked');const ai=await aiModeration(allText,image);if(ai.severe)throw new Error('event_blocked');
    let mod:'published'|'review'=local.status==='review'?'review':'published';let reason=local.reason;if(ai.flagged){mod='review';reason=ai.reason||'ai_flagged'}if(image&&(!ai.available||ai.error)){mod='review';reason='image_requires_manual_review'}const metadata={local:{status:local.status,reason:local.reason},ai:{available:Boolean(ai.available),flagged:Boolean(ai.flagged),severe:Boolean(ai.severe),reason:ai.reason||null,model:(ai as any).model||null,categories:(ai as any).categories||null,scores:(ai as any).scores||null},has_image:Boolean(image)};
    const db=createClient(Deno.env.get('SUPABASE_URL')!,Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,{auth:{persistSession:false}}),now=new Date().toISOString();const profile={telegram_user_id:Number(u.id),username:u.username||null,first_name:u.first_name||'',last_name:u.last_name||null,language_code:u.language_code||null,photo_url:u.photo_url||null,is_premium:Boolean(u.is_premium),allows_write_to_pm:u.allows_write_to_pm??null,updated_at:now,last_auth_at:now};const {error:ue}=await db.from('telegram_users').upsert(profile,{onConflict:'telegram_user_id'});if(ue)throw ue;const row={title,category,event_type:type,starts_at:starts.toISOString(),expires_at:expires,price_rub:price,venue,lat,lng,age_limit:age,description,ticket_url:ticketUrl||null,image_url:image||null,organizer_name:String(u.first_name||'Пользователь').slice(0,80),telegram_owner_id:Number(u.id),promoted:false,moderation_status:mod,moderation_reason:reason,moderation_metadata:metadata,moderated_at:now,going_count:0};const {data,error}=await db.from('events').insert(row).select('id,moderation_status,moderation_reason').single();if(error)throw error;return out({ok:true,...data,moderation_engine:ai.available?'rules+openai-omni':'rules+manual-image-review'});
