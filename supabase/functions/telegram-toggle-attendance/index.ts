...
async function verify(raw:string){
  ...
  const secret=await mac(enc.encode("WebAppData"),token);let ok=same(hex(await mac(secret,check(false))),hash);if(!ok&&p.has("signature"))ok=same(hex(await mac(secret,check(true))),hash);if(!ok)throw new Error("signature_invalid");
...
async function readyTopic(db:any,eventId:string,chatId:number,username:string){
  ...
  if(data?.status==='ready'&&String(data.chat_id)===String(chatId)&&data.message_thread_id)return {available:true,url:`https://t.me/${username}/${Number(data.message_thread_id)}`,thread_id:Number(data.message_thread_id)};
  return {available:false,row:data||null};
}

async function ensureEventChat(db:any,event:any,userId:number){
  const {data:config,error:configError}=await db.from('chaika_app_config').select('forum_chat_id,forum_username').eq('id','telegram_forum').maybeSingle();
  ...
  if(current.row&&String(current.row.chat_id)!==String(chatId)){const {error}=await db.from('event_forum_topics').delete().eq('event_id',event.id);if(error)throw error;current={available:false,row:null};}
  ...
  if(!current.row){
    ...
    const {data,error}=await db.from('event_forum_topics').update({chat_id:chatId,chat_username:username,status:'creating',lease_until:leaseUntil,last_error:null,attempt_count:Number(current.row.attempt_count||0)+1,updated_at:now.toISOString()}).eq('event_id',event.id).eq('status','error').select('event_id').maybeSingle();if(error)throw error;claimed=Boolean(data);
  }else if(current.row.status==='creating'&&(!current.row.lease_until||new Date(current.row.lease_until).getTime()<Date.now())){
    const {data,error}=await db.from('event_forum_topics').update({chat_id:chatId,chat_username:username,status:'creating',lease_until:leaseUntil,last_error:null,attempt_count:Number(current.row.attempt_count||0)+1,updated_at:now.toISOString()}).eq('event_id',event.id).eq('status','creating').lt('lease_until',now.toISOString()).select('event_id').maybeSingle();if(error)throw error;claimed=Boolean(data);
  }
  if(!claimed){for(let i=0;i<8;i++){await sleep(250);const waited=await readyTopic(db,event.id,chatId,username);if(waited.available)return {chat_available:true,chat_url:waited.url,message_thread_id:waited.thread_id};if(waited.row?.status==='error')return {chat_available:false,chat_error:'topic_creation_failed'};}return {chat_available:false,chat_pending:true,chat_error:'topic_creation_pending'};}
  ...
  try{
    ...
    const url=`https://t.me/${username}/${threadId}`,updatedAt=new Date().toISOString();
...
async function safeEventChat(db:any,event:any,userId:number){
  try{return await ensureEventChat(db,event,userId)}catch(error){
    const message=error instanceof Error?error.message:'chat_unavailable';
    console.error('CHAIKA safe event chat',message);
    return {chat_available:false,chat_error:'chat_unavailable'};
  }
}
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==='chat'){
      ...
      const chat=await safeEventChat(db,event,uid);
    ...
    const row=Array.isArray(data)?data[0]:data,going=Boolean(row?.going),goingCount=Number(row?.going_count||0);
