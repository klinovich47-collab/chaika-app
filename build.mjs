...
const peopleOnlyBootRev51 = await readFile('people-only-boot-rev51.js', 'utf8');
...
const productionStableMapRev51 = await readFile('production-stable-map-rev51.js', 'utf8');

// Production is people-only: no organizations catalogue or museum module in the bundle.
await writeFile('dist/index.html', index.replace('./app.js?rev=1', './app.js?rev=51'));
await writeFile('dist/app.js', `${app}\n\n${peopleOnlyBootRev51}\n\n${live}\n\n${enhancements}\n\n${safetyShare}\n\n${mapBasemapGuardRev48}\n\n${mapStress}\n\n${eventChat}\n\n${forumAutoconfigRev12}\n\n${mapUiRev10}\n\n${darkShellRev11}\n\n${interactionStabilityRev13}\n\n${profileActivityRev14}\n\n${safeAreaRev16}\n\n${headerLayoutRev17}\n\n${supportFeedbackRev18}\n\n${sheetGeolocationRev19}\n\n${sheetLayoutRev20}\n\n${eventVisualsRev25}\n\n${userMapPivotRev36}\n\n${realPeopleEventsRev44}\n\n${quickCreateRev37}\n\n${mobilePolishRev39}\n\n${overlaySafeAreaRev40}\n\n${webappLockRev41}\n\n${profileReturnRev42}\n\n${uiRussianPolishRev49}\n\n${productionStableMapRev51}\n`);
console.log('Prepared CHAIKA production build: people-only, one final stable map, entry prompt (rev=51).');
